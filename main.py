...
def task1_layout():
    ...
    for issue in DATA['issues']:
        ...
        open_time = (resolutiondate - created).total_seconds() / 3600 
    ...
    for issue in DATA['issues']:
        ...
        for change in changelog:
            for item in change['items']:
                if item['field'] == 'status':  # Изменения по статусам
        ...
        # Вычисление времени в каждом состоянии
        transitions.sort(key=lambda x: x[2])  # Сортируем по времени
        ...
        # Последнее состояние до закрытия
        if transitions:
...
# Задание 4: Горизонтальная гистограмма по задачам пользователей
def task4_layout():
    issues = DATA['issues']
    
    # Извлекаем исполнителей и репортеров
    user_data = []
    for issue in issues:
    ...
    # Подсчитываем задачи для каждого пользователя в каждой роли
    user_counts = df.groupby(['User', 'Role']).size().reset_index(name='Count')
    
    # Суммируем задачи по пользователям и сортируем по убыванию
...
# Задание 5: Гистограмма залогированного времени
def task5_layout():
    issues = DATA['issues']
    
    # Извлекаем время выполнения (timespent)
    task_data = []
    for issue in DATA['issues']:
...
# Задание 6: График задач по степени серьезности с сортировкой
def task6_layout():
    issues = DATA['issues']

    # Извлекаем информацию о степени серьезности
    ...
    # Устанавливаем порядок серьезности
